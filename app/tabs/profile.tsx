...
export default function Profile() {
  ...
  // Refresh when coming back to Profile tab
  useFocusEffect(
    useCallback(() => {
      loadUser();
      return () => {};
  ...
  // ✅ IMPORTANT: if cleared, show "Not selected" (not Grade 3)
  const grade = user?.grade ? String(user.grade) : T.notSelected;

  // ✅ School name from onboarding, safe fallback
  const school =
  ...
  async function hardReloadApp() {
    // Works in production if expo-updates exists; in dev build DevSettings works
    try {
            ...
            Alert.alert(T.done, T.cleared, [
              {
                text: "OK",
                onPress: () => {
                  // ✅ this forces Home + all tabs to reset & re-read storage
                  hardReloadApp();
                },
              },
            ]);
  ...
  return (
    <SafeAreaView style={styles.root} edges={["top", "left", "right"]}>
      ...
      >
        ...
        {/* MY LEARNING (no streak; includes school) */}
        ...
        <View style={styles.sectionCard}>
          <View style={styles.sectionTop}>
            <View style={[styles.sectionIcon, { backgroundColor: "rgba(251,191,36,0.22)", borderColor: "rgba(251,191,36,0.30)" }]}>
...
const styles = StyleSheet.create({
  ...
  scroller: { flex: 1, backgroundColor: BG },
